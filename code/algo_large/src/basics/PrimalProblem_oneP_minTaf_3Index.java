...
import main.Solution_3Index;

public class PrimalProblem_oneP_minTaf_3Index {
...
//	ArrayList<Integer> forbidden=new ArrayList<Integer>();
	public ArrayList<Integer> nodeSet=new ArrayList<Integer>();
	...
	public PrimalProblem_oneP_minTaf_3Index(Instance para,int pIndex,double initialProdTime,double[] initialLogiTime,ArrayList<Integer> logiTaskIdSetForScheduling) {
		this.para = para;
...
//			q = new IloNumVar[para.numProd + 1][para.numMach][para.maxK];
			...
			y = new IloNumVar[numNodes][numNodes][para.numVeh];// 下标para.logiTaskSet.size()表示depot
//			b = new IloNumVar[numNodes];
//			r = new IloNumVar[numNodes];
			tad = new IloNumVar[numNodes];
			tau = new IloNumVar[numNodes][para.numVeh];
			// 定义变量类型、范围、名称
...
//					r[i] = model.numVar(0, para.batteryCapacity, IloNumVarType.Float, "r[" + i + "]");
						tad[i] = model.numVar(0, Float.MAX_VALUE, IloNumVarType.Float, "tad[" + i + "]");
						tau[i][r]=model.numVar(0, Float.MAX_VALUE, IloNumVarType.Float, "tau[" + i + "," + r + "]");
					}
					for (int j = 0; j < numNodes; j++) {
						if (nodeSet.contains(i) && nodeSet.contains(j)) {
							if (i == j || (i > para.logiTaskSet.size() && j > para.logiTaskSet.size())) {
								y[i][j][r] = model.numVar(0, 0, IloNumVarType.Int, "y[" + i + "," + j + "]");
							} else {
								y[i][j][r] = model.numVar(0, 1, IloNumVarType.Int, "y[" + i + "," + j + "]");
							}
						}
					}
				}
			...
			for (int p = 0; p < para.numProd; p++) {
				for (int m = 0; m < para.numMach; m++) {
					for (int k = 0; k < para.K[p][m]; k++) {
						...
						if (p==pIndex) {
							model.addLe(tad[para.PMKtoLogiTask.get(PMKstr).getId()], tss[p][m][k]);
						}
					}
				}
			}
...
//			}
			...
			for (int r = 0; r < para.numVeh; r++) {
				for (int i : nodeSet) {
					if (i == para.logiTaskSet.size()) {
						...
						for (int j : nodeSet) {
							expr21 = model.sum(expr21, y[j][i][r]);
							expr22 = model.sum(expr22, y[i][j][r]);
						}
			...
			for (int r = 0; r < para.numVeh; r++) {
				...
				for (int j : nodeSet) {
					expr = model.sum(expr,y[para.logiTaskSet.size()][j][r]);
				}
				model.addEq(expr,1);
			}
					...
					for (int j : nodeSet) {
						if (j == para.logiTaskSet.size()) {// 目的地是depot的话跳过
							...
							if (i == para.logiTaskSet.size() && j < para.logiTaskSet.size()) {// 从depot到mach
								Tij = para.getDistance(para.depot, para.ItoLogiTask.get(j).getMach())
										* (1 / para.vehSpeed + 2 * para.consumingRate * para.chargingRate);
								expr4 = model.diff(model.sum(tau[i][r], Tij),
										model.prod(para.bigNum, model.diff(1, y[i][j][r])));
...
//							expr4 = model.diff(model.sum(tau[i], Tij), model.prod(para.bigNum, model.diff(1, y[i][j])));
			...
			for (int r = 0; r < para.numVeh; r++) {
				for (int i : nodeSet) {
					model.addLe(tau[i][r],tad[i]);
				}
				model.addGe(tau[para.logiTaskSet.size()][r], initialLogiTime[r]);
			}
...
//				}
				...
				int numNodes = para.logiTaskSet.size() + 1;// + logiTaskIdSetForScheduling.size();
				yVal = new double[numNodes][numNodes][para.numVeh];
				for (int i:nodeSet) {
					for (int j:nodeSet) {
						for (int r = 0; r < para.numVeh; r++) {
							yVal[i][j][r] = model.getValue(y[i][j][r]);
						}
					}
				}
				tadVal = new double[numNodes];
				for (int i :nodeSet) {
					tadVal[i] = model.getValue(tad[i]);
				}
				tauVal = new double[numNodes][para.numVeh];
				for (int i :nodeSet) {
					for (int r = 0; r < para.numVeh; r++) {
						tauVal[i][r] = model.getValue(tau[i][r]);
					}
				}
...
//				}
				sol = new Solution_3Index(para, xVal, yVal, tauVal, tssVal, tafVal,tadVal);
				sol.parse();
				routes=sol.routes;
//				sol.solCheck();
//				sol.print();
